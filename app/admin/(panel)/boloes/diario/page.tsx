...
export default async function AdminBolaoDiarioPage({
  ...
  const selectedDate = params?.data ?? null;
  const data = await getAdminBoloesDashboardData(selectedDate);
  const card = data.dailyCards.find((item) => item.date === data.selectedDailyDate);
  ...
  return (
    <>
      ...
      <AdminPageTitle
        title={`Bolão diário${data.selectedDailyDate ? ` - ${data.selectedDailyDate}` : ""}`}
        subtitle="Ranking detalhado deste bolão diário, com usuários, posição, cota e pontuação."
      />
      <section className="overflow-hidden rounded-[18px] border border-white/8 bg-[#101010]">
        <div className="border-b border-white/8 px-5 py-4">
          <h2 className="text-[15px] font-black text-white">Ranking do bolão diário</h2>
          <p className="mt-1 text-[12px] font-medium text-white/38">
            {card ? `${card.ticketsCount} cotas · ${card.playersCount} jogadores · ${card.totalPoints} pontos` : "Nenhum bolão diário encontrado"}
          </p>
        </div>
        <BolaoRankingTable rows={data.selectedDailyRanking} emptyText="Nenhuma cota diária ranqueada" />
      </section>
    </>
  );
}
