 ...
 * Step 1: palpite Brasil x Panamá
...
import type { BrasilPanamaPlacarPromoStatus } from "@/lib/promotions/brasil-panama-placar-promo";
import brasilLogo from "@/app/assets/brasil-selecao-logo.png";
import panamaLogo from "@/app/assets/panama-selecao-logo.png";
import bgPromoBrasil from "@/app/assets/bg-promo-brasil.png";

const PROMO_FONT =
  "var(--font-montserrat), ui-sans-serif, system-ui, sans-serif";
const GREEN = "#B1EB0B";
const DISMISS_KEY = "bolao_brasil_panama_placar_dismissed_v2";
const PROMO_Z = 156;
const OPEN_DELAY_MS = 0;
const API_PATH = "/api/promotions/brasil-panama-placar";
...
function OfferStep({
  ...
  return (
    <PromoHeroShell
      heroImage={bgPromoBrasil}
      topPaddingClass={PROMO_HERO_PT_STEP1}
      onClose={onClose}
    >
      <div id="brasil-panama-placar-promo-title" className="text-center">
...
function SuccessStep({
  ...
  return (
    ...
    >
      ...
      <div className="text-center" id="brasil-panama-placar-success-title">
        <div
...
export function BrasilPanamaPlacarPromoHost({
  children,
}: {
  ...
  const [status, setStatus] = useState<BrasilPanamaPlacarPromoStatus | null>(
    null,
  );
  ...
  const refreshStatus =
    useCallback(async (): Promise<BrasilPanamaPlacarPromoStatus | null> => {
      ...
      if (!r.ok) return null;
      return (await r.json()) as BrasilPanamaPlacarPromoStatus;
    }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const data = (await r
        .json()
        .catch(() => ({}))) as BrasilPanamaPlacarPromoStatus & {
        error?: string;
      };
      if (!r.ok) {
  ...
  const modalOverlay =
    showModal && portalReady ? (
      <div
        ...
        aria-labelledby={
          step === "offer"
            ? "brasil-panama-placar-promo-title"
            : "brasil-panama-placar-success-title"
        }
        onClick={handleClose}
      >
