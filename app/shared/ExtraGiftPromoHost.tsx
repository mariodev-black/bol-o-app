...
/**
 * Modal "Brinde extra grátis" — abre só pelo hub de promoções (card).
 *
 *   STEP 1 → oferta (resgate pendente).
 *   STEP 2 → confirmação após POST ou quando já resgatou.
 */
...
function buildClaimedTicketsFromStatus(
  data: ExtraGiftStatus,
): ClaimedTicketRow[] {
  return filterExtraGiftDisplayLeagues(data.leagues)
    .filter((league) => league.alreadyClaimed && league.ticketId)
    .map((league) => ({
      ...
      rodadaNome: league.rodadaNome?.trim() || `${league.rodada ?? 0}ª Rodada`,
    }));
}
...
export function ExtraGiftPromoHost({ children }: { children: React.ReactNode }) {
  ...
  const applyHubOpen = useCallback(
    (data: ExtraGiftStatus) => {
      ...
      if (profileBlocksPromo) {
        toast.error("Complete seu perfil para resgatar promoções.");
        return;
      }

      setStatus(data);

      if (data.canClaim && !data.allClaimed) {
        setClaimedTickets([]);
        setStep("offer");
        setOpen(true);
        return;
      ...
      if (data.allClaimed) {
        const tickets = buildClaimedTicketsFromStatus(data);
        if (tickets.length > 0) {
          setClaimedTickets(tickets);
          setStep("claimed");
          setOpen(true);
          return;
        }
    ...
    [profileBlocksPromo, router, toast],
  );
  ...
  }, [status, toast, playAudio, refreshStatus, invalidatePromotionsHub]);
