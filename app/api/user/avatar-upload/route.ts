...
import { setUserAvatarUploadFromBuffer } from "@/lib/auth/users";
import { responseForDbError } from "@/lib/db-errors";
import { AVATAR_UPLOAD_MAX_BYTES } from "@/lib/user/avatar-upload-storage";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const user = await setUserAvatarUploadFromBuffer(userId, buf, mime);
    if (!user) {
