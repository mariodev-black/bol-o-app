/**
 * Promo "Placar exato — Amistoso Brasil x Panamá".
 *
...
import {
  getBrasilPanamaPlacarPromoStatusForUser,
  submitBrasilPanamaPlacarPromoForUser,
...
export async function GET() {
  ...
  const status = await getBrasilPanamaPlacarPromoStatusForUser(user.id);
...
export async function POST(req: Request) {
  ...
  const result = await submitBrasilPanamaPlacarPromoForUser(
    user.id,
    predCasa,
    predVisitante,
  );
