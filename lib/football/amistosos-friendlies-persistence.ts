...
export async function setAmistososMatchResult(
  ...
  try {
    ...
    const { rowCount } = await client.query(
      ...
      [competitionId, matchId, resultCasa, resultVisitante],
    );
    if (!rowCount) {
    ...
    const { recomputePredictionScoresForMatches } = await import(
      "@/lib/predictions/score-recompute"
    );
    ...
    await client.query("ROLLBACK");
    console.error("[amistosos] setAmistososMatchResult", err);
    return { ok: false, error: "Falha ao salvar placar." };
  } finally {
    client.release();
  }
}
