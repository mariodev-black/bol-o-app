...
async function fetchHubDbSnapshot(
  ...
  const extraSelect = opts.extraGift
    ? `(SELECT COALESCE(
          json_agg(
            json_build_object(
        ...
        FROM tickets t
        INNER JOIN unnest($${paramIdx}::int[], $${paramIdx + 1}::int[]) AS targets(championship_id, rodada)
    ...
    : "NULL::json AS gift_claimed_json";
  ...
  const egitoSelect = opts.egito
    ...
    : "NULL::boolean AS egito_has_bet, NULL::boolean AS egito_submitted";
  ...
  const { rows } = await pool.query<{
    ...
    `SELECT ${extraSelect}, ${egitoSelect}`,
    params,
  );
