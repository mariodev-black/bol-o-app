/**
 * Insere/atualiza palpite Uruguai 2 x 0 Cabo Verde e recomputa pontuação do ticket.
 *
 *   npx tsx --tsconfig tsconfig.scripts.json scripts/fix-ticket-uru-cpv-palpite.ts
 *   npx tsx --tsconfig tsconfig.scripts.json scripts/fix-ticket-uru-cpv-palpite.ts <ticketId> [casa] [visitante]
 */
...
const DEFAULT_SCORE_VISITANTE = 0;
...
async function resolveMatchId(
  ...
  const { rows } = await pool.query<{ match_id: number; home_name: string; away_name: string }>(
    `SELECT match_id, home_name, away_name FROM matches_cache
     WHERE competition_id = $1
       AND (
         (home_name ILIKE '%urug%' AND away_name ILIKE '%cabo%')
         OR (home_name ILIKE '%cabo%' AND away_name ILIKE '%urug%')
         OR (home_name ILIKE '%urug%' AND away_name ILIKE '%verde%')
         OR (home_name ILIKE '%verde%' AND away_name ILIKE '%urug%')
       )
  ...
  if (rows.length === 0) {
    throw new Error(`Partida Uruguai x Cabo Verde não encontrada (comp ${compId})`);
  }
  const uruHome = rows.find(
    (r) => /urug/i.test(r.home_name) && /cabo|verde/i.test(r.away_name),
  );
  const pick = uruHome ?? rows[0]!;
...
async function main() {
  ...
  if (!Number.isFinite(scoreCasa) || !Number.isFinite(scoreVisitante)) {
    console.error(
      "Uso: npx tsx --tsconfig tsconfig.scripts.json scripts/fix-ticket-uru-cpv-palpite.ts [ticketId] [casa] [visitante]",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const uruIsHome = /urug/i.test(match.home_name);
    const palpiteCasa = uruIsHome ? scoreCasa : scoreVisitante;
    const palpiteVisitante = uruIsHome ? scoreVisitante : scoreCasa;
    ...
    console.log(
      `Palpite a gravar (casa x visitante): ${palpiteCasa}x${palpiteVisitante} (Uruguai ${scoreCasa} x ${scoreVisitante} Cabo Verde)`,
    );
