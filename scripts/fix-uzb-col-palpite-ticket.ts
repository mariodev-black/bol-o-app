/**
 * Insere palpite manual UZB 1 x 0 COL e recomputa pontos.
 *
 *   npx tsx --tsconfig tsconfig.scripts.json scripts/fix-uzb-col-palpite-ticket.ts [ticket-id]
 */
...
const TICKET_ID =
  process.argv[2]?.trim() ?? "37f1b92d-2459-4cf0-a4c5-81b5bd9f5ffc";
/** Palpite: 0x1 — Colômbia 1, Uzbequistão 0 */
const SCORE_CASA = 0;
const SCORE_VISITANTE = 1;

async function resolveUzbColMatchId(
  ...
  const { rows } = await pool.query<{ match_id: number }>(
    `SELECT match_id FROM matches_cache
     WHERE competition_id = $1
       AND (
         (home_name ILIKE '%uzb%' OR home_name ILIKE '%uzbe%')
         AND (away_name ILIKE '%col%' OR away_name ILIKE '%colô%')
       )
  ...
  if (id == null) {
    throw new Error(`Partida Uzbequistão x Colômbia não encontrada (comp ${compId})`);
  }
  return Number(id);
}
...
async function main() {
  ...
  try {
    ...
    const matchId = await resolveUzbColMatchId(pool, matchCompId);
    ...
    const matchRes = await client.query<{
      ...
      `SELECT home_name, away_name, status, result_casa, result_visitante, date_br
    ...
    console.log(
      `Partida: ${match.home_name} x ${match.away_name} (#${matchId}, ${match.date_br ?? "?"}) — ${match.status} ${match.result_casa ?? "-"}x${match.result_visitante ?? "-"}`,
    );
